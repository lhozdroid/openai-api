...
public class FineTuneMethod {
  ...
  public String toUrlQueryString(String prefix) {
    ...
    if (getSupervised() != null) {
      try {
        joiner.add(String.format("%ssupervised%s=%s", prefix, suffix, URLEncoder.encode(String.valueOf(getSupervised()), "UTF-8").replaceAll("\\+", "%20")));
      } catch (UnsupportedEncodingException e) {
        // Should never happen, UTF-8 is always supported
        throw new RuntimeException(e);
      }
    ...
    if (getDpo() != null) {
      try {
        joiner.add(String.format("%sdpo%s=%s", prefix, suffix, URLEncoder.encode(String.valueOf(getDpo()), "UTF-8").replaceAll("\\+", "%20")));
      } catch (UnsupportedEncodingException e) {
        // Should never happen, UTF-8 is always supported
        throw new RuntimeException(e);
      }
